...
let days_in_month year month =
  ...
  if is_leapyear year && month = 2 then 29 else days.(month')
...
let is_legal = function
  | Date (yy, mm, dd) ->
      1 <= mm && mm <= 12 && 1 <= yy
      && yy <= 9999 (* otherwise doesn't fit into yyyy-mm-dd *)
      && 1 <= dd
...
let days_since_epoch = function
  | Date (yy, mm, dd) ->
      let y' = yy - 1 in
      let correction =
        if mm <= 2 then 0 else if mm > 2 && is_leapyear yy then -1 else -2
      in
...
let date_from_string str =
  let yy, mm, dd =
    ...
    with Agolex.Error msg -> error "Format error: %s" msg
  in
...
let string_of_date = function
  | Date (yy, mm, dd) -> Printf.sprintf "%04d-%02d-%02d" yy mm dd

(** Difference between two dates in days *)
let diff d1 d2 = days_since_epoch d1 - days_since_epoch d2

let print d1 d2 =
  Printf.printf "%s - %s = %+4d\n" (string_of_date d1) (string_of_date d2)
    (diff d1 d2)
...
let test () =
  let check (date_x, date_y, days) =
    ...
    diff dy dx = days
  in
  if List.for_all check testcases then
...
let main () =
  ...
  | [ x; y ]     -> datediff (date_from_string x) (date_from_string y)
  | _            ->
      usage this stderr;
      exit 0
